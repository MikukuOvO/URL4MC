...
class SoftImputeBase(Generic[T]):
    ...
    def __init__(self, X: T, lambda_: float = 10, threshold: float = 0.001, max_iter: int = 10000,
                 max_rank: Optional[int] = None, numlib: str = "numpy", **kwargs) -> None:
        self.numlib = numlib
    ...
    def shrinkage_operator(self, M: T, max_rank: Optional[int] = None) -> tuple:
    ...
    def solve(self, X: T, missing_mask: T, Z_init: Optional[T] = None) -> T:
    ...
    def fit(self, M: T, missing_mask: T, Z_init: Optional[T] = None) -> T:
        """Complete matrix."""
        return self.solve(M, missing_mask, Z_init=Z_init)

    def fit_transform(self, M: T, missing_mask: T, Z_init: Optional[T] = None) -> T:
        """Complete matrix."""
        return self.fit(M, missing_mask, Z_init=Z_init)

    @abstractmethod
    def _svd(self, X: T, max_rank: Optional[int] = None) -> tuple:
        pass
...
class SoftImpute_N(SoftImputeBase):
    ...
    def _svd(self, M: ndarray, max_rank: Optional[int] = None) -> tuple:
        """Perform SVD."""
        if max_rank and self.svd_randomized:
            LOGGER.debug("Performing randomized SVD.")
            (U, s, V) = fbpca.pca(M, max_rank)
        else:
...
class SoftImpute_T(SoftImputeBase):
    ...
    def _svd(self, M: Tensor, max_rank: Optional[int] = None) -> tuple:
        """Perform SVD."""
        if max_rank and self.svd_randomized:
            (U, s, V) = torch.svd_lowrank(M, max_rank)
            V = V.T
        else:
